...
\emergencystretch=2em   % avoids overfull/stretched lines around long \texttt names
...
\twocolumn[{%
  ...
  \begin{center}
    ...
    {\small St.~Edmund's College \quad\textbar\quad
     \url{https://github.com/aryan35790jp/cross_straight_chinese}\par}
  \end{center}
  \vspace{1.0em}
  \begin{center}
...
encoders that share an identical token vocabulary. Two are trained on Traditional
text, \texttt{ckiplab/\allowbreak bert-base-chinese} and
\texttt{ckiplab/\allowbreak albert-base-chinese} (Academia Sinica \cite{ckip});
three on Simplified text, \texttt{hfl/\allowbreak chinese-bert-wwm-ext},
\texttt{hfl/\allowbreak chinese-macbert-base}, and
\texttt{hfl/\allowbreak chinese-roberta-wwm-ext} \cite{cui2021}. Their pairwise
vocabulary overlap is at least 0.9998, and every one of our 1,816 dataset
characters receives an identical token id in all five. The tokenizer is therefore
a held-constant control: any cross-script difference resides in the learned
weights, not in segmentation.
...
0.10) and significant rather than large; we rely on the calibrated null and the
redundant-control comparison rather than on the effect magnitude. The gloss-based typology is a proxy,
...
\url{https://github.com/aryan35790jp/cross_straight_chinese}.
...
Transformer Models. \url{https://github.com/ckiplab/ckip-transformers}.
